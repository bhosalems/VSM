...
def clip_transformer_forward(model, input_ids_list, attention_mask, class_embed=None):
    ...
    causal_attention_mask = AttentionMaskConverter._make_causal_mask(
...
class MaskEmbedder(nn.Module):
    def __init__(self, conditional_channels=3, out_channels=(16, 64, 256)):
        super().__init__()
        
        self.conv_in = nn.Conv2d(conditional_channels, out_channels[0], kernel_size=3, padding=1)

        self.blocks = nn.ModuleList([])

        for i in range(len(out_channels) - 1):
            channel_in = out_channels[i]
            channel_out = out_channels[i + 1]
            self.blocks.append(nn.Conv2d(channel_in, channel_in, kernel_size=3, padding=1))
            self.blocks.append(nn.Conv2d(channel_in, channel_out, kernel_size=3, padding=1, stride=2))
        
        self.conv_out = self.zero_module(
            nn.Conv2d(out_channels[-1], conditional_channels, kernel_size=3, padding=1, stride=1)
        )

    def forward(self, condition):
        e = self.conv_in(condition)
        e = F.silu(e)
